...
def ChooseLabelWrongTrainingExamples(args, model, tokenizer):
    ...
    file =args.train_data_file
    print("我们要挑出错误分类的文件为：", file)
    ...
    model.to(args.device)
    model.eval()
    folder_file = 'train_wrong_examples.jsonl'
    if os.path.exists(folder_file):
        os.remove(folder_file)
        print("\n已删除原有的数据文件，",folder_file)
    examples=0
    ...
    print("训练错误的数据集生成成功，长度为：", examples)

def test(args, model, tokenizer):
    # 鲁棒性攻击里面用不到了。
    # # 设置两个待测试的文件，一个是含有所有投毒的文件与正常文件，一个是仅含有投毒的文件，用于计算投毒成功率
    # file_list=[args.test_data_file,args.test_backdoor_data_file]
    ...
    for file in file_list:
        ...
        eval_acc = np.mean(labels == preds)
        logger.info("准确率/攻击的成功率是：%s",str(round(eval_acc, 4)))
...
def main(training=True, evaluating=False, testing=False, deletePreviousTrainedModel=True):
    ...
    model.load_state_dict(torch.load(output_dir))
    model.to(args.device)
    ChooseLabelWrongTrainingExamples(args, model, tokenizer)
    # 测试下能否成功打开
    js_all = json.load(open('train_wrong_examples.jsonl'))

if __name__ == "__main__":
    model_type='codeBERT'
    logger.debug("--+++defect dection+++-当前的模型类型为：%s", str(model_type))
    do_train = True
    do_test = True
    # # 先生成数据集
    # # 调用执行Python脚本文件
    ...
    # （1）在原始数据上跑一遍，把训练数据中错误的数据样本挑出来，我们认为这批数据是有问题的，
    # 切记：第一步是在模型训练完成以后，做的
    main(training=True, deletePreviousTrainedModel=False)
