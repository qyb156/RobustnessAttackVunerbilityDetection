...
def get_file_number(filename):
    ...
    try:
        ...
        number_result=random.randint(0,10000)
    return number_result

def gen_random_data(files,filepath,action):
...
def doGetRandomChange(filepath,action):
    files = get_conut_result(filepath)
    # print(files,filepath)
    gen_random_data(files, filepath,action)
